...
class _UsersScreenState extends State<UsersScreen> {
  ...
  List<Map<String, dynamic>> get _filtered {
    ...
    return _users.where((u) =>
        ...
        (u['position'] as String? ?? '').toLowerCase().contains(q)).toList();
  }
                                ...
                                child: Padding(
                                  ...
                                  child: Row(children: [
                                    ...
                                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                      ...
                                      if (u['position'] != null && (u['position'] as String).isNotEmpty)
                                        Text(u['position'] as String,
                                            style: const TextStyle(fontSize: 12, color: BsaTheme.textSecondary)),
                                      const SizedBox(height: 6),
                                      Wrap(spacing: 4, runSpacing: 4, children: roles.map((r) => Container(
                                        padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
                                        decoration: BoxDecoration(
                                          color: _roleColor(r).withOpacity(0.1),
                                          borderRadius: BorderRadius.circular(10)),
                                        child: Text(_roleLabel(r),
                                            style: TextStyle(fontSize: 11, color: _roleColor(r),
                                    ...
                                    if (canManage)
                                      PopupMenuButton<String>(
                                        ...
                                        onSelected: (action) async {
                                          if (action == 'edit') {
                                            await _showUserForm(context, user: u);
                                            _load();
                                        ...
                                        itemBuilder: (_) => [
                                          const PopupMenuItem(value: 'edit', child: Row(children: [
                                            Icon(Icons.edit_outlined, size: 16), SizedBox(width: 8), Text('Editar datos')])),
                                          const PopupMenuItem(value: 'roles', child: Row(children: [
                                            Icon(Icons.manage_accounts_rounded, size: 16), SizedBox(width: 8), Text('Cambiar roles')])),
  ...
  Future<void> _toggleActive(Map<String, dynamic> u) async {
    ...
    final isActive = u['is_active'] as bool? ?? true;
    try {
      await auth.apiService.updateUser(u['id'] as int, {'is_active': !isActive});
      _load();
    } catch (e) {
  ...
  Future<void> _showRolesForm(BuildContext context, Map<String, dynamic> u) async {
    final currentRoles = (u['roles'] as List?)?.map((r) => r.toString()).toList() ?? [];
    final selected = <String>[...currentRoles];

    const allRoles = [
      ('admin', 'Administrador'), ('maintenance_manager', 'Jefe Mantenimiento'),
      ('technician', 'Técnico'), ('warehouse', 'Depósito'),
      ('purchasing', 'Compras'), ('hr', 'RRHH'), ('viewer', 'Visualizador'),
    ];
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final auth = Provider.of<AuthProvider>(context, listen: false);
                try {
                  await auth.apiService.updateUserRoles(u['id'] as int, selected);
  ...
  Future<void> _showUserForm(BuildContext ctx, {Map<String, dynamic>? user}) async {
    ...
    final selectedRoles = <String>[...(user?['roles'] as List?)?.map((r) => r.toString()).toList() ?? ['technician']];
                ...
                children: [
                  ...
                  ...allRoles.map((r) => CheckboxListTile(
                        ...
                        onChanged: (v) => setDState(() {
                          if (v!) selectedRoles.add(r.$1);
                          else selectedRoles.remove(r.$1);
                        }),
                      )),
                ],
              ),
                ...
                try {
                  if (user == null) {
                    ...
                    await auth.apiService.updateUser(user['id'] as int, {
                      ...
                      if (posCtrl.text.isNotEmpty) 'position': posCtrl.text.trim(),
                    });
                    // Actualizar roles si cambiaron
                    await auth.apiService.updateUserRoles(user['id'] as int, selectedRoles);
                  }
                  if (dCtx.mounted) Navigator.pop(dCtx);
                } catch (e) {
                  if (dCtx.mounted) ScaffoldMessenger.of(ctx).showSnackBar(
